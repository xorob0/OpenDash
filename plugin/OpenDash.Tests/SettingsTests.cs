...
namespace OpenDashPlugin.Tests
{
    public class SettingsTests
    {
        ...
        public void The_declared_properties_grow_and_shrink_with_the_rig()
        {
            ...
            settings.Normalise();
            Assert.Equal(Contract.SharedPropertyNames(), settings.DeclaredProperties());
            ...
            Assert.Equal(shared + 2 * perFace + Modules.Count + 6, names.Count);
            ...
            settings.Normalise();
            Assert.Equal(shared + perFace + Modules.Count + 6, settings.DeclaredProperties().Count());
