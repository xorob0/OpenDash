...
describe('at every zone body the build produces, the ids drawn are the ids declared', () => {
  ...
  const UNDECLARED: Record<string, readonly string[]> = {
    ...
    // Zone C of the 600 x 686 face is 112 px, ten less than zone B, and that is where a second row
    // stops fitting at all: the delta loses its scale, lap times its delta, the sectors their three
    // lap times, the session its lap and its time left, the stint its two stops.
    ...
    // And zone B with it, for the same reason as fuel above. Both zones lost two rows to the
    // artboards' frame -- 122 and 112 where the pit wall's gave them 124 and 114 -- and the delta's
    // scale is the row that went with them.
