...
describe('at every zone body the build produces, the ids drawn are the ids declared', () => {
  ...
  const UNDECLARED: Record<string, readonly string[]> = {
    ...
    // The delta's three sector deltas are the rank under the rule, and the rule goes with them.
    // A 469 x 258 zone body holds the number, the bar and the scale and nothing more, and the
    // 600 x 686 face's two zones lose the scale as well. The catalogue draws the sectors at all
    // four of its shapes, so these are boxes it does not draw rather than a drawing withdrawn.
