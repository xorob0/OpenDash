...
Every ticket carries its own `## Dependencies` section, written in a pass on 2026-09-13, recording
what blocks it, what it blocks and what it is worth doing beside. That pass is the detail and this
section is the shape of it, re-checked on 2026-09-21 against the work that has landed since. Where
the two disagree, read the ticket: a body is amended when the graph moves, and this page is a map
rather than a second source.

Two groups carry no such section, both having been filed after the pass. The twenty-three of the
readability pass are the first, and their order is set out below because it is real and unstated.
The four filed between 16 and 21 September are the second.

### The keystones

Counting only dependants that are themselves still open:

| Ticket | Unblocks | Why it is a keystone |
|---|---|---|
| [#195](https://github.com/xorob0/OpenDash/issues/195) | 24 | `tokens.json` becomes a base and a set of overlays. Nothing themed can be built against a single flat token file |
| [#196](https://github.com/xorob0/OpenDash/issues/196) | 23 | A face stops being five fixed parts, the anatomy becoming something a theme declares |
| [#194](https://github.com/xorob0/OpenDash/issues/194) | 22 | Trade dress. It is a licence question rather than an engineering one, and no car ticket may merge before it |
| [#200](https://github.com/xorob0/OpenDash/issues/200) | 19 | The conformance harness, which is what makes "a theme changes the register, never the catalogue" a fact rather than an intention |
| [#107](https://github.com/xorob0/OpenDash/issues/107) | 6 | ADR 0010, where alert priority is decided, and with it who owns band D at a given instant |
| [#310](https://github.com/xorob0/OpenDash/issues/310) | 6 | Where a per-game difference is resolved |
| [#311](https://github.com/xorob0/OpenDash/issues/311) | 5 | The mapping layer: one property name per game, decided once rather than per binding |
| [#130](https://github.com/xorob0/OpenDash/issues/130) | 5 | The brand document as the default rather than the law, which every personalisation ticket needs settled |
| [#146](https://github.com/xorob0/OpenDash/issues/146) | 4 | Retiring the card path, which is the hinge of the tidy |
| [#145](https://github.com/xorob0/OpenDash/issues/145) | 4 | What a round face does with zones |
| [#127](https://github.com/xorob0/OpenDash/issues/127) | 4 | Colour as the user's, with one rule that stays |

The first four are worth reading together. The Car themes milestone holds seventy-five tickets and
not one of them is mergeable until all four have landed, so the whole milestone is four tickets deep
rather than seventy-five wide. Moreover [#194](https://github.com/xorob0/OpenDash/issues/194) is the
cheapest of the four to start and the only one that needs no code at all.

### The ordering hazards no single ticket owns

**A rename in the middle of the readability pass.**
[#151](https://github.com/xorob0/OpenDash/issues/151) moves `second/` and `modules/` to a neutral
home, and its own note warns that doing it before the rest of wave 04 would make each of those a
rename conflict. The larger collision is with the readability pass, which was filed after that note
was written. Of its twenty-three tickets, eighteen edit a page under `modules/` and the two mechanisms
edit `second/table.ts` and `second/layout.ts`, which leaves only the parent and the two that draw in
`zones/` untouched by the move. The rename therefore has to happen before the pass opens or after it
closes, and never while it is in flight.

**The snapshot nobody can read.**
`packages/dash/test/__snapshots__/snapshots.test.ts.snap` is 60,945 lines of the repository's
63,079, and twenty-two commits rewrote it between 15 and 21 September.
[#123](https://github.com/xorob0/OpenDash/issues/123) is the ticket that makes a page's snapshot
readable, and CONTRIBUTING asks that a changed snapshot be read before it is refreshed, which is not
a thing anybody does to a file that size. It is recorded as blocked by
[#151](https://github.com/xorob0/OpenDash/issues/151), a digest keyed by module path being written
twice if the paths are about to be renamed, so the chain is the rename, then the snapshot, then the
pass. Should that ordering prove too expensive, the honest alternative is to accept unreadable
snapshot diffs for the duration of the pass and to say so, rather than to leave the rule standing
and unobserved.

**An idiom that is wrong only sometimes, and has spread.**
[#226](https://github.com/xorob0/OpenDash/issues/226) reports that spreading `withBindings` over an
element replaces the bindings the element built for itself rather than adding to them. The ticket
names five call sites. On 2026-09-21 there are thirty-five files, so the trap grew by thirty while
the ticket sat in wave 02, and every one of them is harmless only for as long as the element it is
spread over has no bindings of its own. It is a two-line fix to `bind.ts` plus a mechanical sweep,
and it becomes more expensive with every ticket that adds a thirty-sixth.

**The deletion, and the release that has to precede it.**
[#146](https://github.com/xorob0/OpenDash/issues/146) removes the card path in one pull request,
because a deletion is only revertible if it is whole. Three things have to be true first, and the
ticket records all three: [#170](https://github.com/xorob0/OpenDash/issues/170) has to ship in the
release *before* it, since `Slot01` to `Slot12` are published properties that an LED profile may
read; [#145](https://github.com/xorob0/OpenDash/issues/145) has to be decided or `480round.ts` and
`800round.ts` stop compiling; and [#212](https://github.com/xorob0/OpenDash/issues/212) is easier to
answer while both paths exist, `cards/position.ts` reading `PositionMode` the same way. One
consequence is not recorded on either ticket: eight of the eighteen packages `bun run shots`
photographs are `openDash slots` faces, which is the whole of the discrepancy
[#190](https://github.com/xorob0/OpenDash/issues/190) reports, so after the deletion the count is
ten and that ticket's help text is correct without being touched.

**The formatter and the deletion, in that order and no other.**
[#119](https://github.com/xorob0/OpenDash/issues/119) turns the house rules into checks. It is
placed after [#146](https://github.com/xorob0/OpenDash/issues/146) because a formatter run over a
tree that is about to lose `cards/`, `slots.ts` and `rung.ts` with their sixteen importers produces
a diff nobody can read alongside the deletion's, and before
[#195](https://github.com/xorob0/OpenDash/issues/195) because the theme layer's standing rule is
that no hex value escapes the token file, which is exactly the check this ticket writes.

### The readability pass has an order, and the tickets do not state it

[#327](https://github.com/xorob0/OpenDash/issues/327) is the parent and closes when its children do.
The two mechanisms come first, and one of them is already done:
[#329](https://github.com/xorob0/OpenDash/issues/329) gave `stack()` its justify and closed on
2026-09-21, while [#328](https://github.com/xorob0/OpenDash/issues/328) is half landed, the cell
ramp now answering the row height and no caller yet declaring one.

That remaining half sits under four of the page tickets, which say so in prose and not in a
dependency line: [#339](https://github.com/xorob0/OpenDash/issues/339),
[#340](https://github.com/xorob0/OpenDash/issues/340) and
[#343](https://github.com/xorob0/OpenDash/issues/343) cite it directly, and
[#341](https://github.com/xorob0/OpenDash/issues/341) is the same table asked a different question.
Doing any of the four before [#328](https://github.com/xorob0/OpenDash/issues/328) means deciding a
row count in one page that the mechanism will then have to generalise.

Two further pairings are worth knowing. Lap history has two halves, the missing fuel target being
[#326](https://github.com/xorob0/OpenDash/issues/326) and the layout being
[#343](https://github.com/xorob0/OpenDash/issues/343), and the header the target belongs in is drawn
by the second. The driver name is owned jointly by
[#172](https://github.com/xorob0/OpenDash/issues/172), which asks what a name does when it will not
fit, and [#328](https://github.com/xorob0/OpenDash/issues/328), which asks how large the column
should be in the first place; answering the second first makes the first a smaller question.

Finally, [#331](https://github.com/xorob0/OpenDash/issues/331) is worth re-reading before it is
started. It names [#222](https://github.com/xorob0/OpenDash/issues/222) as the other half of the
delta page and says the two stay separate, and
[#222](https://github.com/xorob0/OpenDash/issues/222) has since been built, so the page it describes
is no longer the page in the tree.

### The four filed after the dependency pass

[#353](https://github.com/xorob0/OpenDash/issues/353) and
[#366](https://github.com/xorob0/OpenDash/issues/366) both require an amendment to
[ADR 0018](decisions/0018-the-cars-own-lights.md), the first for whether a screen adopts the car's
colours and the second for whether the tables are fetched on a press or on a schedule. Two
amendments to one record written a week apart is how a record becomes a changelog, so they are
better answered in one sitting even if the code lands separately. Note also that
[#353](https://github.com/xorob0/OpenDash/issues/353) is explicit that it is not worth starting
until its question is chosen.

[#363](https://github.com/xorob0/OpenDash/issues/363) is unblocked and small: `LedTargets` already
enumerates LED devices and resolves the id a `LedBar` records, so the matrix wants the same list
rather than a new mechanism.

[#362](https://github.com/xorob0/OpenDash/issues/362) is blocked on SimHub rather than on anything
here, and its body names the three APIs that would unblock it. Half of it returned in 0.3.0-rc.4.

### What the triage of 2026-09-21 changed

Four tickets lost their last blocker: [#71](https://github.com/xorob0/OpenDash/issues/71) and
[#74](https://github.com/xorob0/OpenDash/issues/74) through the cancellation of
[#72](https://github.com/xorob0/OpenDash/issues/72),
[#145](https://github.com/xorob0/OpenDash/issues/145) through
[#232](https://github.com/xorob0/OpenDash/issues/232),
[#210](https://github.com/xorob0/OpenDash/issues/210) through
[#141](https://github.com/xorob0/OpenDash/issues/141) and
[#325](https://github.com/xorob0/OpenDash/issues/325) through
[#306](https://github.com/xorob0/OpenDash/issues/306). Fifteen more had been free for longer without
anybody noticing, [#96](https://github.com/xorob0/OpenDash/issues/96),
[#147](https://github.com/xorob0/OpenDash/issues/147) and
[#152](https://github.com/xorob0/OpenDash/issues/152) among them.

One edge was removed rather than satisfied.
[#167](https://github.com/xorob0/OpenDash/issues/167) was recorded as blocking four tickets that
each needed to show something for three seconds, and all four have since been built without it,
reading a duration off the lap that has started rather than off a clock. It gates nothing now, and
what remains of it is a recording rather than a spike.

**The Status column in the table below is a snapshot of the migration and is not maintained.** It
was correct on 2026-09-15 and is wrong for the twenty-seven tickets closed on 2026-09-21. The issue
itself is the status.
